...
if parent/ancestor messaging is disabled, ordinary descendant messages to any ancestor/addressee in their hierarchy are still delivered as full context-only `Info` messages: the addressed ancestor sees the message and receives it in memory-channel context, but it is not prompted. Verified reverse replies to an earlier routed message bypass this downgrade and are delivered according to their original `Reply`/`Type` semantics. Unrelated contexts are rejected.
